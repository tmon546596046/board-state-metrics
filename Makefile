...
IMAGE = $(REGISTRY)/board-state-metrics
...
doccheck:
	...
	@grep -hoE '(board_[^ |]+)' docs/* --exclude=README.md| sort -u > documented_metrics
	@sed -n 's/.*# TYPE \(board_[^ ]\+\).*/\1/p' pkg/collectors/*_test.go | sort -u > tested_metrics
...
docker-push: .docker-push-$(ARCH)
.docker-push-$(ARCH): .container-$(ARCH)
...
clean:
	rm -f board-state-metrics

.PHONY: all build all-push all-container test-unit container docker-push clean validate-modules
